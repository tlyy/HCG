...
class BBFAgent(dqn_agent.JaxDQNAgent):
  ...
  def __init__(
    ...
    self.ndrf_itv = ndrf_itv
    self.last_av = {'projection/net': onp.zeros(2048)}
    self.last_count = {'projection/net': onp.zeros(2048)}
    self.total_count = {'projection/net': onp.zeros(2048)}
    logging.info(self.ndrf)
  ...
  def _build_networks_and_optimizer(self):
    ...
    self.cum_grad = jax.tree_map(lambda p: jnp.zeros_like(p), self.online_params)
  ...
  def reset_weights(self):
    ...
    if not self.reset_head:
      keys_to_copy += ["head"]
    keys_to_copy = ['encoder', 'transition_model', 'projection', 'predictor', 'kernell', 'biass', 'head']
    # keys_to_copy = ['kernell', 'biass']
  ...
  def _training_step_update(self, step_index, offline=False):
    ...
    intermediates = get_intermediates(self.network_def, self._support, new_online_params, self.replay_elements['state'][:, 0])
    if not hasattr(self, "cum_intermediates"):
      self.cum_intermediates = intermediates
    else:
      self.cum_intermediates = jax.tree_map(
        lambda acc, itm: acc + jnp.abs(itm),  # 累加绝对值
        self.cum_intermediates, 
        intermediates
      )
    self.cum_grad = jax.tree_map(
      lambda acc, g: acc + jnp.abs(g),  # 累加绝对值
      self.cum_grad, 
      grad
    )
    # self.cum_grad = grad
    # if self.ndrf:
    #   if self.training_steps % self.ndrf_itv == 0:
    #       (
    #         new_online_params,
    #         new_target_params,
    #         new_optimizer_state, 
    ...
    #   write_log(intermediates, self.cum_grad, new_online_params, self.last_av, self.last_count, self.dead_neuron_threshold, self.base_dir)
    # if self.training_steps % 1000 == 0:
    #   print(self.last_av['projection/net'] / self.last_count['projection/net'])
    #   logging.info(self.total_count['projection/net'][:5])
  ...
  def recycle_weights(self):
    # intermediates = get_intermediates(self.network_def, self._support, self.online_params, self.replay_elements['state'][:, 0])
    write_log(self.cum_intermediates, self.cum_grad, self.online_params, self.last_av, self.last_count, self.dead_neuron_threshold, self.base_dir)
    (
      self.online_params,
      self.target_network_params,
      self.optimizer_state, 
    ...
    self.cum_grad = jax.tree_map(lambda p: jnp.zeros_like(p), self.online_params)
    self.cum_intermediates = jax.tree_map(lambda p: jnp.zeros_like(p), self.cum_intermediates)
    logging.info('recycle weights')
    self.cycle_grad_steps = 0
  ...
  def _train_step(self):
    ...
    if self._replay.add_count > self.min_replay_history:
      ...
      if self.training_steps % self.ndrf_itv == 0 and self.training_steps < 100000:
        self.recycle_weights()
